addRemoteEvents{"Shop->Weapons->UI"};--addEvent


addEventHandler("Shop->Weapons->UI",root,function(typ)
	...
	if(typ=="Open")then
		...
		GUI.Window[1]=dgsCreateWindow(10,GLOBALscreenY/2-600/2,400,600,SHOPS["Weapons"].Name,false,tocolor(255,255,255),GUI.Settings.Height,nil,GUI.Color.Bar,nil,GUI.Color.BG,nil,true);
		...
		--weapons
		GUI.Tab[1]=dgsCreateTab("Weapons",GUI.Tabpanel[1],1.3,1.3,tocolor(255,255,255),nil,tocolor(0,0,0,140),nil,nil,nil,_,GUI.Color.Bar,GUI.Color.Bar,_,_,tocolor(255,255,255,255),tocolor(255,255,255,255),tocolor(120,0,0,255));
		GUI.Grid[1]=dgsCreateGridList(10,10,360,425,false,GUI.Tab[1],30,GUI.Color.Grid.BG,tocolor(255,255,255,255),GUI.Color.Grid.Bar,tocolor(65,65,65,255));
		...
		local itemName=dgsGridListAddColumn(GUI.Grid[1],"Item",0.45);
		local itemPrice=dgsGridListAddColumn(GUI.Grid[1],loc("UI->Shop->Price"),0.35);
		local itemLevel=dgsGridListAddColumn(GUI.Grid[1],loc("UI->Shop->Level"),0.2);
		
		for _,v in pairs(SHOPS["Weapons"].ItemsWeapons)do
			...
			dgsGridListSetItemText(GUI.Grid[1],row,itemLevel,v[3],false,false);
		end
		...
		--ammo
		GUI.Tab[2]=dgsCreateTab("Ammo",GUI.Tabpanel[1],1.3,1.3,tocolor(255,255,255),nil,tocolor(0,0,0,140),nil,nil,nil,_,GUI.Color.Bar,GUI.Color.Bar,_,_,tocolor(255,255,255,255),tocolor(255,255,255,255),tocolor(120,0,0,255));
		...
		for _,v in pairs(SHOPS["Weapons"].ItemsAmmo)do
			...
			function(btn,state)
				if(btn=="left" and state=="up")then
					...
					if(item>0)then
						...
						if(clicked~="")then--string.find(amount,".")
							local amount=dgsGetText(GUI.Edit[2]);
							if(amount~=nil and amount~="" and isOnlyNumber(amount)and #amount>0)then
								triggerServerEvent("Shop->Buy->Ammo",localPlayer,tostring(clicked),math.floor(amount));
							end
						end
					end
				end
			...
			function(btn,state)
				if(btn=="left" and state=="up")then
					...
					if(item>0)then
						...
						if(clicked~="")then
							if(clicked=="Armor")then
								triggerServerEvent("Shop->Buy->Item",localPlayer,tostring(clicked),1);
							else
								triggerServerEvent("Shop->Buy->Weapon",localPlayer,tostring(clicked));
							end
						end
					end
				end
		...
		dgsSetProperty(GUI.Edit[2],"placeHolder","Enter amount");
